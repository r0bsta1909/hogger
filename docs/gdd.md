...
- **Hogger-Tracker (Runde 8, #108):** Liegt Hogger außerhalb des sichtbaren Kreises, erscheint ein dezentes Mini-Hogger-Medaillon am Innenrand des Rings in seiner Richtung — man weiß immer, wo er ist, und kann sich auf ihn zubewegen. Sichtbar solange er lebt (auch auf dem Leash-Rückweg), kein Blinken; nach dem Fluchbruch verschwindet er.
...
- **Ein Try** beginnt mit Leeroys Signal (Kap. 10) und endet mit Hoggers Tod ODER nach **15:00 auf der hochzählenden Uhr** ODER **wenn Hogger zurücksetzt** (Runde 9, #117) — dann leasht Hogger zurück, heilt voll, Leeroy sagt den Wipe an ("Okay. Das war nichts. Nächster Try."), die Statistik-Tafel erscheint kurz (Kap. 11), der Try-Zähler tickt hoch, weiter geht's. Kein Menü, keine Lobby zwischen Trys — der Fluch macht keine Pausen.
- **Reset = Try-Abbruch** (Runde 9): Beide Reset-Gründe aus 9.1 — Leash (600 px, 2 s) und **Kein-Kontakt** (20 s ohne erreichtes Ziel, `hogger_no_contact_reset`) — beenden den Try und werten ihn wie einen Wipe; die Rest-HP auf der Tafel wird VOR dem Full Heal gelesen. Wer Hogger wegzieht oder ihn ins Leere laufen lässt, beendet damit den Try, statt ihn auszusitzen. **Die Kein-Kontakt-Uhr läuft nur im Kampf und nur, solange lebende, nicht verstohlene Spieler da sind; ein toter Raid hält sie an** — die Todesspirale (13.1) bleibt der Weg, einen Try zu verlieren, und Hogger behält dabei seine Rest-HP. Fressen und Charge pausieren die Uhr, setzen sie aber nicht zurück.
...
AGGRO/MELEE → RUSHING CHARGE (CD bereit + Ziel) → FRESSEN (Leiche in 200 px + HP < 90 % + CD)
  → RESET (Leash: Distanz > 600 px durchgehend 2 s [Hysterese] ODER Kein-Kontakt:
     20 s im Kampf ohne erreichtes lebendes Ziel) → TRY-ABBRUCH: Tafel, Zähler +1,
     Hogger steht mit vollen HP wieder am Hügel (Kap. 6). Während CHARGE ist die
     Leash-Prüfung ausgesetzt; CHARGE und FRESSEN pausieren die Kein-Kontakt-Uhr.
     Ein toter Raid hält die Kein-Kontakt-Uhr an (Attrition, 13.1).
...
| Leash | 600 px | Kiten unmöglich; Reset beendet seit Runde 9 den Try (Kap. 6) |
| Kein-Kontakt-Reset | 20 s (`hogger_no_contact_reset`) | Erreicht Hogger im Kampf so lange kein lebendes Ziel, setzt er zurück und der Try gilt als abgebrochen — Kiten und Stalling beenden den Try, statt ihn auszusitzen. Uhr läuft nur im Kampf und nur bei lebenden Zielen (toter Raid hält sie an); Fressen und Charge pausieren sie |
...
- **Abbruch-Gefühl (Runde 9):** Fühlt sich ein Try-Abbruch durch Hoggers Reset fair an, oder reißt er Trys mitten aus dem Kampf? Messbar im Log (`hogger_reset` je Abend, nach Ursache) — der Regler dafür ist `hogger_no_contact_reset` im F10-Panel.
...
| Gruppe | Parameter |
|---|---|
| Hogger | HP-Steigung und HP-Sockel (HP = Steigung × N − Sockel), Autohit-Schaden/-Intervall, Cleave-Divisor, Tempo, Slice-Werte, Charge-CD/-Schaden/-Anlauf, Leash, Kein-Kontakt-Reset |
